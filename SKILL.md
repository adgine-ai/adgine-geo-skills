...
description: >
  ...
  引用测试 / 内容生成 / 网站审计 / 品牌画像 / WordPress 发布 / 流量分析 / 网站流量 / PV / UV /
...
凡是“查看、查询、分析、对比、盘点、状态、趋势、报告、导出”等只读数据请求，必须先路由到
`adgine/geo-reports`。场景默认格式为 HTML 时，**本轮必须立即运行报告命令并交付 HTML 链接**：
不得先用专项读取脚本回答、不得只给文字摘要、不得询问用户是否还要生成报告。账号信息、项目列表、
Worker 部署、单个 SaaS 任务和单个机会等小结果默认直接在对话中输出。用户明确指定 HTML、inline
或 JSON 时始终服从用户。专项技能中的读取脚本仅用于低层排障，不得在报告完成后重复调用同一批
接口做二次摘要。
...
“查询竞争对手/竞品排名/所有竞品”必须使用 `competitor-rankings`，不得用项目配置接口的竞争对手列表
代替分析排名。当前排名接口最多返回 100 个品牌；只总结实际返回的记录，不得把数据库候选量、执行次数
或其他 `total` 字段说成竞争对手数量。
...
用户询问网站整体流量、用户数、UV、PV、会话数、跳出率、停留/会话时长或流量渠道时，使用
`adgine/geo-reports website-traffic`；用户同时要求整体流量和 AI 获客表现时使用 `traffic-overview`。
纯 AI 引荐问题仍使用 `ga4-overview` / `ga4-referrals`。整体 GA4、GA4 AI 引荐和 Cloudflare AI
数据必须分区展示，禁止跨来源相加；所有报告都不得展示收入或交易数据。

报告命令输出 `REPORT_INDEX` / `REPORT_ITEM` 时，必须在对话正文中按原编号、原顺序展示这些条目，
即使完整数据已经写入 HTML。每轮最多展示 40 条，并明确提示用户可以继续说“查询 #2 …”；完整清单仍以
HTML 为准。后续的 `#N` 只指向助手最近一次输出的编号清单，不是数据库 ID，也不是指标排名；应使用该条目
中已给出的名称、域名、路径或内容调用对应查询场景。编号不存在或已被更新时先请用户重新指定，禁止猜测。
